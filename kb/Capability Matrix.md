...
| `embassy-ted` | EU TED procurement notices + `LiveTedProvider` (cursor pagination) | EU public-tender feed | Live opportunities for B2G use cases. The `live` feature POSTs the v3 search endpoint and follows the `iterationNextToken` cursor via `manifold::pagination::paginate` until the caller's `limit` is met (default cap: 50 pages × 100 = 5,000 notices). Supports `TedRequest::Lookup` and `SearchByCountry`. |
...
| `pagination` (`pagination` feature) | Closure-driven `paginate(backend, initial, advance, config)` | One pagination helper, every strategy | Works with cursor / offset / RFC 5988 Link header — the caller's advance closure decides. Silent truncation under `max_pages` is a typed error (`PaginationError::MaxPagesReached`), not a quiet "we saw everything" — audit-grade. First consumer: `embassy-ted` follows TED's `iterationNextToken` chain. |
