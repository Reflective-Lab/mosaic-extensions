...
- **Embassy ports ship STUB ONLY today** — first-pass audit (2026-05-22) found that every P0 crate (`ofac-sls`, `gleif`, `linkedin`, `sec-edgar`, `bolagsverket`, `vies`, `eu-sanctions`, `commerce-csl`, `sam-gov`, `usaspending`, `ted`, `skatteverket`) currently ships only a `Stub*Provider`. No live HTTP / live-data implementation exists in any of them. This means *no scenario or app can satisfy this Standard for Embassy steps until a real provider lands*. Surfaced by `atelier-showcase/scenarios/counterparty-kyc-convergence` (2026-05-22), which refuses to run without `--mock-ok` for this reason. Two next moves: (a) build `HttpGleifProvider` against `https://api.gleif.org/api/v1/lei-records` (free public API, no auth needed) as the first live provider; (b) build an OFAC SDN provider over the published feed (`https://www.treasury.gov/ofac/downloads/sdn.csv`). Both close G1 in `~/dev/reflective/marquee-apps/shoal-meta/kb/portfolio-stretch.md`.
