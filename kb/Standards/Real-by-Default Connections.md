...
- **Embassy ports — partial closure 2026-05-22** — first-pass audit found that every P0 crate (`ofac-sls`, `gleif`, `linkedin`, `sec-edgar`, `bolagsverket`, `vies`, `eu-sanctions`, `commerce-csl`, `sam-gov`, `usaspending`, `ted`, `skatteverket`) shipped only a `Stub*Provider`. **First live providers landed today**: `embassy-sec-edgar` (`LiveSecEdgarProvider` behind the `live` feature) and `embassy-gleif` (`LiveGleifProvider` behind the `live` feature; verified end-to-end against `api.gleif.org` returning Apple Inc.'s real `LegalEntity` payload through the arena scenario at `arena-tests/crates/counterparty-kyc-convergence`). The remaining ten P0 ports are still stub-only. **Next moves** to keep closing G1: OFAC SDN over the published feed (`https://www.treasury.gov/ofac/downloads/sdn.csv`) — that closes the second leg of the regulated-onboarding screening triad and lets the arena scenario flip to REAL LIVE end-to-end.
