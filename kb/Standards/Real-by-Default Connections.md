...
- ~~**`soter-smt`** — `FakeSmtBackend` exposed in default builds.~~ **CLOSED 2026-05-21**: renamed to `ScriptedSmtBackend` and gated behind a non-default `fake-backend` feature. Default-features builds have *no* concrete backend — consumers must pick `cvc5` (real) or `fake-backend` (test) by intent. Integration test moved to `required-features = ["fake-backend"]`; CI updated.
- ~~**`manifold-adapters` LLM ctors** — `OpenAiBackend::new(api_key) -> Self` infallible.~~ **CLOSED 2026-05-21**: all seven primary LLM backends (openai, gemini, mistral, openrouter, staik, kong, anthropic) now expose `try_new(api_key) -> BackendResult<Self>` (replacing the infallible `new`); both `try_new` and `from_env` validate that the API key is non-empty / non-whitespace and emit a loud `BackendError::Unavailable` naming the env var. The arcee wrapper now propagates `OpenAiBackend::try_new(...)?`. minmax and writer already exposed only `from_env` and were compliant. All workspace tests and clippy clean.
- **Embassy ports — broader closure 2026-05-22** — six of twelve P0 crates now ship live providers behind a `live` cargo feature, all following the same shape (manifold `HttpFetchProvider`, fallible ctor with no silent stub fallback, canned-fixture unit tests + `#[ignore]` integration test for the live network call): `embassy-sec-edgar` (LiveSecEdgarProvider), `embassy-gleif` (LiveGleifProvider → api.gleif.org), `embassy-ofac-sls` (LiveOfacSlsProvider → treasury.gov SDN.CSV), `embassy-eu-sanctions` (LiveEuSanctionsProvider → OpenSanctions mirror of EU FSF; canonical EU-Login endpoint configurable), `embassy-commerce-csl` (LiveCommerceCslProvider → OpenSanctions mirror of US Trade CSL; canonical trade.gov endpoint configurable), `embassy-usaspending` (LiveUsaspendingProvider → api.usaspending.gov). The regulated screening triad (OFAC + EU + Commerce) is now complete in code. The arena scenario `arena-tests/crates/counterparty-kyc-convergence` runs REAL LIVE end-to-end and has been verified against the real SDN ("GAZPROM" → fuzzy hit on "GAZPROMBANK JOINT STOCK COMPANY"). **Remaining stub-only P0 ports**: `linkedin` (restrictive ToS, paid API gate), `bolagsverket` (limited free tier), `vies` (free SOAP/REST, next), `sam-gov` (free api_key required), `ted` (open feed, manageable), `skatteverket` (limited public access).
