...
- **Embassy ports — 9 of 12 P0 live as of 2026-05-23** — `embassy-sec-edgar`, `embassy-gleif`, `embassy-ofac-sls`, `embassy-eu-sanctions`, `embassy-commerce-csl`, `embassy-usaspending`, `embassy-vies`, `embassy-sam-gov`, `embassy-ted`. TED is the first port to use `manifold::pagination::paginate` (cursor-based via `iterationNextToken`); the request enum gained a `SearchByCountry` variant to give the helper a real consumer. **Remaining stub-only P0 ports**: `linkedin` (restrictive ToS, paid API gate), `bolagsverket` (limited free tier), `skatteverket` (limited public access).

- ~~**Manifold pagination is the next platform feature**~~ **CLOSED 2026-05-23**: `manifold::pagination::paginate` shipped behind the `pagination` feature flag. Closure-driven so consumers pick cursor / offset / link-header strategy via the advance closure rather than a baked-in enum. `PaginationError::MaxPagesReached` surfaces silent-truncation risk as a typed error rather than letting partial pulls masquerade as complete. First consumer is `embassy-ted::LiveTedProvider`.
