...
- **Embassy ports — 7 of 12 P0 live as of 2026-05-23** — `embassy-sec-edgar`, `embassy-gleif`, `embassy-ofac-sls`, `embassy-eu-sanctions`, `embassy-commerce-csl`, `embassy-usaspending`, `embassy-vies`. The vies provider went live today using new POST + XML support in manifold (see next entry). The regulated screening triad (OFAC + EU + Commerce) is verified end-to-end via the arena scenario. **Remaining stub-only P0 ports**: `linkedin` (restrictive ToS, paid API gate), `bolagsverket` (limited free tier), `sam-gov` (free api_key required), `ted` (open feed, manageable; would benefit from manifold pagination), `skatteverket` (limited public access).

- **Manifold pagination is the next platform feature** — three of the remaining ports (TED procurement notices, USAspending bulk award queries, OFAC SDN once it grows past 8 MiB) want a uniform pagination helper rather than each port hand-rolling cursor / offset / link-header iteration. Sketch: `manifold::pagination::PaginatedFetcher` takes a `WebFetchRequest` template + a `Strategy` (cursor, offset, RFC 5988 Link header) + a stop predicate, and returns an async stream of typed bodies. Documented here so the next port that needs it (TED) is the pull that lands the feature, not its absence.
