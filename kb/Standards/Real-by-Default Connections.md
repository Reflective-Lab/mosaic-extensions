...
- **Embassy ports — 8 of 12 P0 live as of 2026-05-23** — `embassy-sec-edgar`, `embassy-gleif`, `embassy-ofac-sls`, `embassy-eu-sanctions`, `embassy-commerce-csl`, `embassy-usaspending`, `embassy-vies`, `embassy-sam-gov`. SAM.gov is the first port to require a free api_key (`SAM_GOV_API_KEY` from `api.data.gov/signup/`); `from_env()` fails loud with an actionable diagnostic when the var is unset, never silent stub fallback. **Remaining stub-only P0 ports**: `linkedin` (restrictive ToS, paid API gate), `bolagsverket` (limited free tier), `ted` (open feed, manageable; would benefit from manifold pagination), `skatteverket` (limited public access).
